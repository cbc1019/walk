...
-- (기존 테이블이 이미 있다면 아래 한 줄로 부서 컬럼 추가)
ALTER TABLE steps ADD COLUMN IF NOT EXISTS dept TEXT;
...
-- 반환 컬럼이 바뀌므로 기존 함수를 먼저 삭제 후 재생성
DROP FUNCTION IF EXISTS get_period_ranking(DATE, DATE);
CREATE OR REPLACE FUNCTION get_period_ranking(p_start DATE, p_end DATE)
RETURNS TABLE(employee_id TEXT, name TEXT, dept TEXT, total_steps BIGINT)
